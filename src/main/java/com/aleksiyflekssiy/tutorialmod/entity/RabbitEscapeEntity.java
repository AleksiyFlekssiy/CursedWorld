...
import com.aleksiyflekssiy.tutorialmod.entity.ai.RabbitEscapeAI;
...
public class RabbitEscapeEntity extends Shikigami{
    ...
    public boolean doHurtTarget(Entity pEntity) {
        System.out.println("Before Hurted");
        float damage = (float) this.getAttributeValue(Attributes.ATTACK_DAMAGE);
        System.out.println("Hurted");
        return pEntity.hurt(this.damageSources().mobAttack(this), damage);
    }
    ...
    public boolean followOrder(LivingEntity target, BlockPos blockPos, IOrder order) {
        if (super.followOrder(target, blockPos, order)){
            ...
            if (order == RabbitEscapeOrder.ATTACK){
                ...
                this.brain.setMemory(MemoryModuleType.WALK_TARGET, new WalkTarget(blockPos, 1, 1));
            }
            return true;
        }
        return false;
