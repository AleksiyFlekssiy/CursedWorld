package com.aleksiyflekssiy.cursedworld.item.custom;
...
public class UnlimitedVoidItem extends Item {
    ...
    public UnlimitedVoidItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!domainActive) {
            if (!CursedEnergyCapability.isEnoughEnergy(player, 50)) return InteractionResultHolder.fail(stack);
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int slot, boolean selected) {
        if (!level.isClientSide() && entity instanceof Player player) {
            ...
            if (domainActive && domainCenter != null) {
                ...
                if (domainTicks >= DOMAIN_DURATION || checkBarrierDamage(level)) {
                    for (LivingEntity trapped : trappedEntities) {
                        trapped.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(0.1);
                        if (trapped instanceof Player affectedPlayer) {
                            affectedPlayer.getAbilities().mayBuild = true;
                            affectedPlayer.getAbilities().mayfly = player.getAbilities().instabuild;
                        }
                    }
            ...
            if (player.level() instanceof ServerLevel serverLevel) {
                for (Player serverPlayer : serverLevel.players()) {
                    for (ItemStack stack : serverPlayer.getInventory().items) {
                        if (stack.getItem() instanceof UnlimitedVoidItem domainItem) {
                            if (domainItem.isDomainActive() &&
                                    domainItem.getTrappedEntities().contains(player) &&
                                    player != domainItem.getDomainOwner()) {
                                System.out.println("WORKING!!!");
                                event.setCanceled(true);
                                return; // Выходим после первой активной территории
                            }
                        }
                    }
                }
