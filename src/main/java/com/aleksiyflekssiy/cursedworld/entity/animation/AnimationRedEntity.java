package com.aleksiyflekssiy.cursedworld.entity.animation;
...
import static com.aleksiyflekssiy.cursedworld.util.RotationUtil.getOffsetLookPosition;

public class AnimationRedEntity extends Entity {
    private final Player owner;
    private static final EntityDataAccessor<Boolean> START_FUSION = SynchedEntityData.defineId(AnimationRedEntity.class, EntityDataSerializers.BOOLEAN);

    public AnimationRedEntity(EntityType<?> pEntityType, Level pLevel) {
        this(pEntityType, pLevel, null);
    }

    public AnimationRedEntity(EntityType<?> pEntityType, Level pLevel, Player player) {
    ...
    public void tick() {
        if (owner != null && owner.isAlive()) {
            ...
            if (!entityData.get(START_FUSION)) {
                ...
                Vec3 currentPos = this.position();
                Vec3 delta = targetPos.subtract(currentPos).scale(1); // Плавное приближение
                move(MoverType.SELF, delta);
            }
