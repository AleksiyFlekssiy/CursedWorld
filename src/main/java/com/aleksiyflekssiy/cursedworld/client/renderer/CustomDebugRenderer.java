...
public class CustomDebugRenderer {
    public static final List<AABB> AABB_LIST = new ArrayList<>();
    ...
    public static void addOBB(OBB obb, boolean bool){
        OBB_LIST.put(obb, bool);
    }

    public static void removeAABB(AABB aabb){
        AABB_LIST.remove(aabb);
    }
    ...
    public static void debugRender(RenderLevelStageEvent event) {
        ...
        for (OBB box : OBB_LIST.keySet()) {
            ...
            float red = 1;
            float green = 0;
            if (box.contains(mc.player.position())){
                green++;
                red--;
            }
            // 12 рёбер OBB
        ...
        OBB_LIST.keySet().removeIf(obb -> {
            if (OBB_LIST.get(obb) == false) {
